...
class VolatilityReconstructor:
    ...
    Edges: Direct 4-Point Quadratic Anchor Extrapolation.
    Transform: Log-Moneyness for perfect parabolic mapping.
    """
    def __init__(self, filepath):
        ...
        # Edge Anchor Parameters (Original 4-point Quadratic)
    ...
    def _get_quadratic_prediction(self, x_arr, y_arr, target_x):
        """Degree-2 Polynomial for the Interior Baseline AND Edges"""
    ...
    def process_surface(self):
        ...
        for exp, meta in self.chain_map.items():
            for r_idx in tqdm(self.raw_df.index, desc=f"Reconstructing {exp}"):
                ...
                if pd.isna(S) or S <= 0:
                    continue # Skip bad spot rows, fallback handles them later
                ...
                for opt_type in ['CE', 'PE']:
                    ...
                    # --- LOG-MONEYNESS UPGRADE HERE ---
                    # Replaced (strike / S) with np.log(strike / S)
                    known_x = np.array([np.log(strikes[cols.index(c)] / S) for c in known_cols])
                    ...
                    for m_col in missing_cols:
                        t_x = np.log(strikes[cols.index(m_col)] / S)
