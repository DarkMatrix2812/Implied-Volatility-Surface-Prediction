...
class VolatilityReconstructor:
    ...
    Edges: Direct Anchor Extrapolation (Linear, 4 points).
    Post-Processing: Temporal Savitzky-Golay Smoothing.
    """
    def __init__(self, filepath):
        self.dataset_path = filepath
        self.fallback_vol = 1e-6
        
        # Cross-Sectional Blending Logic
        self.quad_weight = 0.25
        self.pchip_weight = 0.75
        
        # Edge Anchor Parameters (Linear to prevent explosions)
        self.anchor_points = 4 
    ...
    def _get_edge_prediction(self, x_arr, y_arr, target_x):
        """Degree-1 Linear Fit for Edges to Prevent Parabola Explosions"""
        if len(y_arr) == 0: return np.nan
        if len(y_arr) == 1: return self._clip(y_arr[0])
        try:
            coefs = np.polyfit(x_arr, y_arr, 1) # Force Linear
            return self._clip(np.polyval(coefs, target_x))
        except Exception:
            return np.nan
                ...
                for opt_type in ['CE', 'PE']:
                    ...
                    for m_col in missing_cols:
                        ...
                        # --- LEFT EDGE (LINEAR EXTRAPOLATION) ---
                            ...
                            val = self._get_edge_prediction(anchor_x, anchor_y, t_x)
    ...
    def save_and_submit(self):
        print("\nApplying Savitzky-Golay Temporal Smoothing...")
        # Temporal Smoothing Pass down the columns (Time Axis)
        for col in self.opt_cols:
            if len(self.filled_df[col]) > self.sg_window and not self.filled_df[col].isna().any():
                smoothed_values = savgol_filter(
                    self.filled_df[col].values, 
                    window_length=self.sg_window, 
                    polyorder=self.sg_poly
                )
                # Clip to prevent negative volatilities
                self.filled_df[col] = np.clip(smoothed_values, self.fallback_vol, np.inf)
