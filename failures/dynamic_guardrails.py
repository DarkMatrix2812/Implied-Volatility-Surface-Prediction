...
class VolatilityReconstructor:
    ...
    def __init__(self, filepath):
        ...
        # Blending Logic
        self.quad_weight = 0.285
        self.pchip_weight = 0.715
        
        # Edge Anchor Parameters
        self.anchor_points = 4  # Use the 4 nearest known points to anchor the edge curve
        
        # --- EXPERIMENTAL DYNAMIC GUARDRAILS ---
        self.use_dynamic_guardrails = True
        self.guardrail_multiplier = 1.10 # Caps extrapolated wings at 110% of max known IV
        ...
        for exp, meta in self.chain_map.items():
            ...
            for r_idx in tqdm(self.raw_df.index, desc=f"Reconstructing {exp}"):
                ...
                for opt_type in ['CE', 'PE']:
                    ...
                    # Extract only the KNOWN data points (our anchors)
                    ...
                    # Calculate the Dynamic Guardrail Limit for this specific curve
                    max_known_iv = np.max(known_y)
                    guardrail_limit = max_known_iv * self.guardrail_multiplier
                    
                    # Sort to ensure left-to-right processing
                    ...
                    for m_col in missing_cols:
                        ...
                        # --- LEFT EDGE EXTRAPOLATION ---
                            ...
                            # Anchor to the first N known points
                            ...
                            # Apply the Guardrail ceiling if enabled
                            if self.use_dynamic_guardrails and np.isfinite(val):
                                val = min(val, guardrail_limit)
                            
                        # --- RIGHT EDGE EXTRAPOLATION ---
                        else:
                            # Anchor to the last N known points
                            anchor_x = known_x[-self.anchor_points:]
                            anchor_y = known_y[-self.anchor_points:]
                            val = self._get_quadratic_prediction(anchor_x, anchor_y, t_x)
    ...
    def save_and_submit(self):
        self.filled_df.to_csv('filled_dataset_with_guardrails.csv', index=False)
        rows = []
        ...
        sub.to_csv('submission_guardrails.csv', index=False)
        print(f"submission_guardrails.csv: {len(sub)} rows | NaN={sub['value'].isna().sum()} | range=[{sub['value'].min():.5f}, {sub['value'].max():.5f}]")
